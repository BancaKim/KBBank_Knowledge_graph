---
name: KB 친환경 매직카대출(신차)
category: 자동차대출
amounts:
  max: 6천만원
channels:
- 영업점
page_url: https://obank.kbstar.com/quics?page=C103429&cc=b061496:b061645&isNew=N&prcode=LN20001389
page_id: C103429
scraped_at: '2026-03-23T13:55:49'
term: 최장10년
repayment: 상환방법  분할상환
eligibility_summary: "신차구입 목적으로 자동차판매회사(대리점)와 매매계약을 체결하고 소득증빙이 가능한 만 19세 이상 개인고객\n\
  ...
  \ 사용연료가 전기, 수소전기, 하이브리드(휘발류+전기)를 사용하는 친환경차\n         (개인택시, 캠핑카 및 "
---
...
캐피탈 대비 유리한 금리조건으로 친환경차량 신차를 구매하려는 고객을 위한 맞춤상품
...
※ 대출대상 : 근로소득자(6개월 이상), 사업소득자(12개월 이상), 연금소득자
※ 대상차종 : 승용차, 승합차, 화물차(5톤 이하)로 사용연료가 전기, 수소전기, 하이브리드(휘발류+전기)를 사용하는 친환경차
         (개인택시, 캠핑카 및 카라반, 수입차 포함, (대형)이륜자동차 미포함) 
*친환경차량 : 전기, 수소전기, 하이브리드(휘발류+전기)를 연료로 사용하는, 「환경친화적 자동차의 개발 및 보급 촉진에 관한 법률」, 같은 법 시행령 및 「환경친화적 자동차의 요건 등에 관한 규정」에 명시된 차량
...
「KB 친환경매직카대출(신차)」을 통해 KB신용/체크카드 결제방식 이용시 대출금액의 0.5%~1.5% 캐시백 지급
...
※ 신차 구매자금을 KB 매직카대출(신차)를 통한 KB국민카드 결제방식과 KB국민카드사 전용 특별 한도를 중복 신청하는 경우 승인시점에 따라 KB 매직카대출(신차) 국민카드 결제방식으로 처리되지 않을 수 있으므로 유의하여 주시기 바랍니다.
...
③ 우대금리 : 최고 연 1.5%p 우대
 ...
 ※ 실적연동 우대금리는 각 항목의 우대조건의 충족여부에 따라  대출신규 3개월 이후 매월 재산정되어 적용됩니다.
  ☞영업점 우대 : 최고 연 0.7%p(전체 항목모두해당되어도 최고 연 0.7%p 적용)
   -	신용등급(CSS)1~3등급 고객 : 연 0.2%p
   -	KB스타클럽(베스트 이상)우대 : 연 0.2%p 
   -	친환경차 우대 : 연 0.5%p 
   * 마일드 하이브리드는 하이브리드 차량이 아닙니다. 
...
※ 대출 실행 전① 본인확인서류② 재직 및 소득증빙서류③ 자동차 구매 관련 확인서류- 신차구매 : 자동차매매계약서 ※ 이륜자동차 : 매매계약서, 제2종 소형면허증※ 대출 실행 후- 구매자금대출 : (본인 소유로 명의 이전 확인 가능한) 자동차등록원부
