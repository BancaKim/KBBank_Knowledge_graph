...
| `ltv_calculator.py` | `calculate_ltv_limit` - LTV 기반 대출 한도 계산 (규제지역/비규제/생애최초, 주택가격대별 상한) |
| `mortgage_calculator.py` | `calculate_mortgage_limit` - 통합 대출한도 = min(LTV한도, DSR한도), 상환방식별/기간별 비교 |
